...
技术方案固定为前后端分离：后端 Node.js + Express 提供认证 API，前端 React + Less + Ant
...
**Primary Dependencies**: Express, mysql2, bcrypt, jsonwebtoken, cors, dotenv, React, React Router, antd, less  
...
- 路由守卫 `ProtectedRoute`：无 token 或校验失败时重定向到登录页。
...
- 登录页：表单校验 -> 调用登录 API -> 成功后存 token + 拉取 `/me` -> 跳转工作台。
- 注册页：表单校验（用户名唯一格式、密码强度、二次确认）-> 调用注册 API -> 成功后跳转登录或自动登录。
- 全局拦截：401/403 时清理本地登录态并跳转登录页，展示统一错误提示。
- 登出流程：点击登出 -> 调用登出 API -> 清理前端状态 -> 回到登录页。
...
- 设计完成后再次核验：Node.js + React + Less/antd + MySQL 均保持一致，**PASS**。
